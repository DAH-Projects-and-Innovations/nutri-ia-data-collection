...
> **Important** : le projet utilise un environnement virtuel géré par `uv`. Si tu lances `python main.py ...` directement, Python ne trouvera pas les dépendances (loguru, torch, etc.).

**Option 1 — via `uv run` (recommandé, sans activation)**
...
```bash
# 0. Copier les images brutes dans le dossier source
cp -r /chemin/vers/mes/images/ data/raw/images/
# (ou manuellement via Finder → data/raw/images/)

# 1. Prétraiter les images raw → processed
#    (vérification, conversion RGB, redimensionnement optionnel)
uv run python main.py prepare

# 2. Générer les embeddings des images processed
uv run python main.py embed
```

Options avancées pour `prepare` :
